...
use NetBrothers\KostentraegerParser\SgbV\Institution;
use NetBrothers\KostentraegerParser\SgbV\Verknuepfung;
...
final class SonstigeParser extends AbstractParser
{
	protected function parseAssociations(): void
	{
		...
		foreach ($this->segments as $segment) {
			...
			$this->institutions[$currentIkNumber]->associate(new Verknuepfung(
				...
				AbrechnungscodeEnum::tryFrom($segment[9] ?? ''),
				TarifkennzeichenTarifbereichEnum::tryFrom($segment[10] ?? ''),
				$segment[11] ?? null
			));
		}
	}
